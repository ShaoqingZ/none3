...
class Sam3Image(torch.nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.prompt_tuning_mode = prompt_tuning_init_text is not None
        if self.prompt_tuning_mode:
            for param in self.parameters():
                param.requires_grad = False
        self.pt_text = prompt_tuning_init_text
        self.pt_phi = None 
        self.register_buffer("pt_base_embeds", None)
    ...
    def initialize_phi_parameter(self):
        if self.pt_text is None:
            return
        ...
        with torch.no_grad():
            context_len = text_encoder.context_length
            
            token_ids = text_encoder.tokenizer(
                [self.pt_text], 
                context_length=context_len
            ).to(device)

            if hasattr(text_encoder, 'encoder'):
                base_embeds = text_encoder.encoder.token_embedding(token_ids).detach()
            else:
                base_embeds = text_encoder.token_embedding(token_ids).detach()

        self.pt_phi = nn.Parameter(torch.zeros_like(base_embeds))

        self.pt_phi.requires_grad = True
        
        self.pt_base_embeds = base_embeds.clone()
    ...
    def forward(self, input: BatchedDatapoint):
        ...
        if self.prompt_tuning_mode and self.pt_phi is not None:
            
            tuned_embeds = self.pt_base_embeds + self.pt_phi
            text_outputs = self.backbone.forward_text(
                captions = [self.pt_text],
                device=self.device,
                input_embeds=tuned_embeds 
            )
        else:
            text_outputs = self.backbone.forward_text(input.find_text_batch, device=self.device)
