...
def main(args) -> None:
    ...
    if args.override_data:
        cfg.trainer.data.train.dataset.img_folder = args.train_img_dir
        cfg.trainer.data.train.dataset.ann_file.input_json_path = args.train_json_path
        cfg.launcher.experiment_log_dir = args.output_path
        cfg.trainer.logging.log_dir = os.path.join(args.output_path, "logs")
        cfg.trainer.checkpoint.save_dir = os.path.join(args.output_path, "checkpoints")
        cfg.trainer.logging.tensorboard_writer.log_dir = os.path.join(args.output_path, "tensorboard")
        
        cfg.trainer.continual.phi_checkpoint_dir = args.output_path
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        required=True,
        type=str,
        help="path to config file (e.g. configs/polyp/prompt.yaml)",
    )
    parser.add_argument(
        "--use-cluster",
        type=int,
    ...
    parser.add_argument(
        "--override_data", 
        action="store_true", 
        help="Enable dynamic overriding of data paths from CLI"
    )
    parser.add_argument("--train_img_dir", type=str, help="Absolute path to training images")
    parser.add_argument("--train_json_path", type=str, help="Absolute path to training annotation json")
    parser.add_argument("--output_path", type=str, help="Absolute path to save outputs (checkpoints/logs)")
    parser.add_argument("--epochs", type=int, default=-1, help="Override max epochs")
