...
class Trainer:
    ...
    def __init__(
        ...
        *,  # the order of these args can change at any time, so they are keyword-only
        ...
        max_epochs: int,
        continual: Optional[Dict[str, Any]] = None,
        mode: str = "train",
        ...
        self.continual_conf = ContinualConf(**continual) if continual is not None else ContinualConf()
        ...
        self.phi_save_path = os.path.join(
            self.continual_conf.phi_checkpoint_dir,"phi_continuous_checkpoint.pt"
        )
        self.phi_load_path = os.path.join(
            self.continual_conf.phi_load_dir,"phi_continuous_checkpoint.pt"
        )
    ...
    def _init_sccm_anchor(self, model):
        if hasattr(self, 'anchor_embedding') and self.anchor_embedding is not None:
            return
        device = self.device
        model_unwrapped = unwrap_ddp_if_wrapped(model)
        
        was_training = model.training
        model.eval()

        with torch.no_grad():
            phi_path = self.phi_load_path
            if os.path.exists(phi_path):
                logging.info(f"Mode: Continuous Training.")
                try:
                    saved_phi = torch.load(phi_path, map_location=device)
                    if hasattr(model_unwrapped, 'pt_phi'):
                        if model_unwrapped.pt_phi.shape == saved_phi.shape:
                            model_unwrapped.pt_phi.data.copy_(saved_phi)
                            logging.info("Successfully loaded saved phi weights into model.")
                        else:
                            logging.error(f"Shape mismatch: Model {model_unwrapped.pt_phi.shape} vs Saved {saved_phi.shape}")
                    else:
                        logging.error("Model does not have 'pt_phi' attribute.")

                except Exception as e:

                    logging.error(f"Failed to load phi checkpoint: {e}")
    
        if was_training:
            model.train()
            
    def compute_sccm_loss(self, model):
        if not hasattr(self, 'anchor_embedding') or self.anchor_embedding is None:
            self._init_sccm_anchor(model)
            
        if self.anchor_embedding is False or self.anchor_embedding is None:
            return torch.tensor(0.0, device=self.device)

        model_unwrapped = unwrap_ddp_if_wrapped(model)

        if hasattr(model_unwrapped, 'pt_phi') and model_unwrapped.pt_phi is not None:
            current_phi = model_unwrapped.pt_phi 
            base_embeds = model_unwrapped.pt_base_embeds.detach()
            
            # shape: [1, 32, 1024]
            current_tuned_embeds = base_embeds + current_phi
            current_tuned_embeds = current_tuned_embeds / current_tuned_embeds.norm(dim=-1, keepdim=True)
            
            target = self.anchor_embedding.to(self.device)
            
            if current_tuned_embeds.shape != target.shape:
                if target.dim() == 2:
                     target = target.unsqueeze(0)
            
            loss_mse = torch.nn.MSELoss()
            loss_sccm = loss_mse(current_tuned_embeds, target)
            return loss_sccm
            
        return torch.tensor(0.0, device=self.device)


    def _save_phi_checkpoint(self):
        if self.distributed_rank != 0 or not self.continual_conf.enabled: return

        model_unwrapped = unwrap_ddp_if_wrapped(self.model)
        if hasattr(model_unwrapped, 'pt_phi'):
            try:
                phi_data = model_unwrapped.pt_phi.detach().cpu()
                torch.save(phi_data, self.phi_save_path)
            except Exception as e:
                logging.error(f"Failed to save phi: {e}")
    ...
    def train_epoch(self, train_loader):
        ...
        extra_loss_mts = {}
        extra_loss_mts["anchor_loss"] = AverageMeter("anchor_loss", self.device, ":.4f")
        ...
        if self.continual_conf.enabled:
            self._init_sccm_anchor(self.model)
        ...
        is_tta_mode = getattr(model_unwrapped, 'prompt_tuning_mode', False)
        should_use_cache = enable_cache and is_tta_mode and (iters_per_epoch < 5)

        iterator = None

        if should_use_cache:
            if not hasattr(self, '_tta_cached_batch'):
                self._tta_cached_batch = None
                
            if self.epoch == 0:
                try:
                    self._tta_cached_batch = next(iter(train_loader))
                except StopIteration:
                    return {} 
            
            if self._tta_cached_batch is None:
                self._tta_cached_batch = next(iter(train_loader))


            iterator = [(0, self._tta_cached_batch.copy())]
        else:
            iterator = enumerate(train_loader)

        for data_iter, batch in iterator:
    ...
    def _run_step(
        ...
        for i, chunked_batch in enumerate(batch):
            ...
            with ddp_context:
                ...
                assert len(loss_dict) == 1
                loss_key, loss = loss_dict.popitem()
                sccm_loss = torch.tensor(0.0, device=self.device)
                # if self.continual_conf.enabled:
                #     sccm_loss = self.compute_sccm_loss(self.model)*self.continual_conf.sccm_lambda
                # loss = loss + sccm_loss
                ...
                if "sccm_loss" not in extra_loss_mts:
                    extra_loss_mts["sccm_loss"] = AverageMeter("sccm_loss", self.device, ":.4f")
                extra_loss_mts["sccm_loss"].update(sccm_loss.item(), batch_size)
