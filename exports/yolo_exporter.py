...
class YOLOExporter:
    ...
    def export(self, project: dict, output_dir: str,
        ...
        # 创建目录结构
        (yolo_path / 'images').mkdir(parents=True, exist_ok=True)
        (yolo_path / 'labels').mkdir(parents=True, exist_ok=True)
        (yolo_path / 'masks').mkdir(parents=True, exist_ok=True)
        (yolo_path / 'visualizations').mkdir(parents=True, exist_ok=True)
        ...
        for img in all_images:
            ...
            # 1. 绝对严格模式：只有你亲手点过“保存”或“确认完成”（处于已完成池），才允许导出！
        ...
        for img_info in images:
            result = self._export_image(
                img_info, output_path, class_to_id
            )
            if result:
                total_annotations += result
    ...
    def _export_image(self, img_info: dict, output_path: Path,
        ...
        # 复制图片
        dst_image = output_path / 'yolo' /'images' / filename
        ...
        label_file = output_path / 'yolo' /'labels' / f"{name_without_ext}.txt"
        lines = []
        ...
        for ann in annotations:
            ...
            if self.format_type == 'segment' and ann.get('polygon'):
                ...
                if len(polygon) >= 3:
                    ...
                    cv2.fillPoly(mask_img, [pts], 255) # 二值 Mask 画 255 白色
        ...
        mask_file = output_path / 'yolo' /'masks' / f"{name_without_ext}.png"
        cv2.imwrite(str(mask_file), mask_img)

        cv2.addWeighted(overlay, 0.4, vis_img, 0.6, 0, vis_img)
        vis_file = output_path / 'yolo' /'visualizations' / f"{name_without_ext}.jpg"
...
#             (output_path / 'visualizations' / split).mkdir(parents=True, exist_ok=True)
...
#         mask_file = output_path / 'masks' / split / f"{name_without_ext}.png"
#         cv2.imwrite(str(mask_file), mask_img)

#         cv2.addWeighted(overlay, 0.4, vis_img, 0.6, 0, vis_img)
#         vis_file = output_path / 'visualizations' / split / f"{name_without_ext}.jpg"
#         cv2.imwrite(str(vis_file), vis_img)
