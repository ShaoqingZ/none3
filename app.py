...
from services.al_service import ActiveLearningService
...
# 初始化 AL 服务
al_service = ActiveLearningService(sam3_service, annotation_manager)
...
def create_project():
    ...
    project = {
        ...
        'images': [],
        'current_index': 0,
        'text_prompt': data.get('text_prompt', 'A polyp in a colonoscopy image'),
        'image_states': {}
    }
    annotation_manager.create_project(project)
    try:
        al_service.generate_project_training_config(project_id)
    except Exception as e:
        print(f"配置文件生成失败: {e}")
    return jsonify({'success': True, 'project': project})

@app.route('/api/projects/<project_id>/next-batch', methods=['POST'])
def get_next_batch(project_id):
    batch = al_service.fetch_next_manual_batch(project_id)
    return jsonify({"success": True, "batch": batch})

@app.route('/api/projects/<project_id>/train_and_infer', methods=['POST'])
def start_train_and_infer(project_id):
    """【一键融合】：触发 优化提示微调 + 机器自动检验"""
    data = request.json or {}
    thresh = float(data.get('solved_thresh', 0.4))
    epochs = int(data.get('epochs', 50))
    lr = float(data.get('lr', 0.0005))
    
    # 调用新的融合后台任务
    al_service.run_train_and_infer_bg(project_id, thresh, epochs, lr)
    
    return jsonify({"success": True, "message": "训练与推理流程已在后台启动"})

@app.route('/api/projects/<project_id>/al_progress', methods=['GET'])
def get_al_progress(project_id):
    """获取主动学习后台任务进度"""
    task_status = getattr(al_service, 'al_tasks', {}).get(project_id)
    if not task_status:
        return jsonify({"success": True, "data": {"status": "idle"}})
    return jsonify({"success": True, "data": task_status})

@app.route('/api/projects/<project_id>/images/<image_name>/state', methods=['PUT'])
def update_image_state_api(project_id, image_name):
    data = request.json or {}
    new_state = data.get('state')
    annotation_manager.update_image_state(project_id, image_name, new_state)
...
def delete_project(project_id):
    ...
    try:
        ...
        temp_config_path = os.path.join("SAM_src", "sam3", "train", "configs", "temp", f"prompt_{project_id}.yaml")
