...
function updateImageList() {
    ...
    if (total === 0) {
        const emptyHtml = '<div class="empty-state"><i class="bi bi-images"></i><p>暂无图片</p></div>';
        const poolEl = document.getElementById('listPool');
        if (poolEl) poolEl.innerHTML = emptyHtml;
        return;
    }

    state.images.forEach((img, idx) => {
        const imgState = imageStates[img.filename] || 'unannotated';

        // 构建 DOM 元素 (保留原有类名以便复用原有CSS，同时加入状态标识)
        const item = document.createElement('div');
        item.className = `image-item ${idx === state.currentIndex ? 'active' : ''} ${img.annotated ? 'annotated' : ''}`;
        item.onclick = () => loadImage(idx);

        let badgeHtml = '';
        if (imgState === 'review') badgeHtml = '<span class="badge bg-warning text-dark ms-2" style="font-size:0.7rem;">待审</span>';
...
async function loadImage(index) {
    ...
    currentImage.onload = () => {
        ...
        // === 主动学习新增：控制打回/确认按钮的显示 ===
        const imgState = state.projectData?.image_states?.[image.filename] || 'unannotated';
        const btnReject = document.getElementById('btnReject');
        const btnAccept = document.getElementById('btnAccept');
        const btnSaveNormal = document.getElementById('btnSaveNormal'); // 原有的普通保存按钮

        if (btnReject && btnAccept) {
            if (imgState === 'review') {
                btnReject.style.display = 'inline-block';
                btnAccept.style.display = 'inline-block';
                if(btnSaveNormal) btnSaveNormal.style.display = 'none'; // 隐藏普通保存
            } else {
                btnReject.style.display = 'none';
                btnAccept.style.display = 'none';
                if(btnSaveNormal) btnSaveNormal.style.display = 'inline-block'; // 恢复普通保存
            }
        }
        // ===========================================
    };
...
async function saveAnnotations(showMessage = true) {
    ...
    try {
        ...
        if (data.success) {
            ...
            if (state.annotations.length > 0) {
                // 如果图上有标注，说明你确认了金标准，将其移入已完成池
                state.projectData.image_states[imgName] = 'completed';
                fetch(`/api/projects/${state.projectId}/images/${imgName}/state`, {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ state: 'completed' })
                });
                if (showMessage) showToast('成功', '金标准已保存，图片已移入已完成池！');
            } else {
                // 如果你把图上的标注全清空了并点了保存，它应该退回未标注池
                state.projectData.image_states[imgName] = 'unannotated';
                fetch(`/api/projects/${state.projectId}/images/${imgName}/state`, {
                    method: 'PUT',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ state: 'unannotated' })
                });
                if (showMessage) showToast('成功', '标注已清空，图片已退回未标注池！');
            }
            updateImageList();
            if (showMessage) {
                showToast('成功', '标注已保存');
...
async function runTrainAndInfer() {
    ...
    const slider = document.getElementById('threshSlider');
    const thresh = slider ? slider.value : 0.6;

    if (!confirm(`即将使用您刚刚标注的图作为先验知识优化模型，随后模型将自动扫描所有未标注图片（阈值：${thresh}）。该过程可能需要几分钟，是否继续？`)) return;

    try {
        // 第一步：先显示转圈圈动画 (传入 progress = -1 会触发你的 loading-pulse CSS 动画)
        showLoading('正在生成配置并启动模型训练，请耐心等待...', -1);
        
        // 触发后台任务
        await fetch(`/api/projects/${state.projectId}/train_and_infer`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ solved_thresh: thresh })
        });
...
async function changeImgState(newState) {
    ...
    try {
        ...
        const btnReject = document.getElementById('btnReject');
        const btnAccept = document.getElementById('btnAccept');
        const btnSaveNormal = document.getElementById('btnSaveNormal');
        
        if (newState === 'review') {
            if(btnReject) btnReject.style.display = 'inline-block';
            if(btnAccept) btnAccept.style.display = 'inline-block';
            if(btnSaveNormal) btnSaveNormal.style.display = 'none';
        } else {
            if(btnReject) btnReject.style.display = 'none';
            if(btnAccept) btnAccept.style.display = 'none';
            if(btnSaveNormal) btnSaveNormal.style.display = 'inline-block';
        }
    } catch (error) {
...
async function rejectImage() {
    await changeImgState('unannotated');
    showToast('已打回', '该图片已被退回未标注池！', 'info');
}

// 5. 确认完成并保存
async function acceptImage() {
    await changeImgState('completed');
    await saveAnnotations(false);  // 复用原有的保存逻辑，不弹多余的框
