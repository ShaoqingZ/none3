...
async function clearCurrentAnnotations() {
    ...
    if (confirm(`确定要清除当前图片的 ${state.annotations.length} 个标注吗？`)) {
        ...
        redraw();
        // 自动保存
        await saveAnnotations(false,true);
        showToast('成功', '已清除并保存');
    }
}
...
function onKeyDown(e) {
    ...
    else if (e.key === 'v' || e.key === 'V') toggleVisibility();
}
...
function redraw() {
    ...
    if (state.showAnnotations) {
        ...
        state.tempPoints.forEach(p => drawPoint(p.x, p.y, p.label));
        if (state.tempPolygon.length > 0) drawTempPolygon();

        ctx.restore();

        state.tempBoxes.forEach(box => drawTempBox(box.x1, box.y1, box.x2, box.y2, box.label));
    }
}
...
function quickRedraw(dynamicBox) {
    if (!currentImage || !staticCanvas) {
        redraw();
        // ====== 增加判断 ======
        if (dynamicBox && state.showAnnotations) {
            drawTempBox(dynamicBox.x1, dynamicBox.y1, dynamicBox.x2, dynamicBox.y2);
        }
        return;
    }

    // 从缓存绘制静态内容（静态缓存内部已经处理了是否显示Mask）
    ctx.clearRect(0, 0, canvas.width, canvas.height);
    ctx.drawImage(staticCanvas, 0, 0);

    // ====== 新增判断：如果开启了显示，才画动态画笔、临时点、和正在拖动的框 ======
    if (state.showAnnotations) {
        // 绘制动态内容
        ctx.save();
        ctx.scale(state.zoom, state.zoom);
...
async function segmentByPoints() {
    ...
    try {
        ...
        if (data.success && data.results.length > 0) {
            ...
            updateAnnotationList();
            redraw();
            await saveAnnotations(false,true);  // 自动保存
                ...
                <div class="project-item-main" onclick="selectProject('${p.id}')">
                    <div class="d-flex justify-content-between align-items-center">
                        <div>
                            ...
                            <code class="ms-2 text-primary" style="font-size: 0.75rem; background: #f0f7ff; padding: 2px 4px; border-radius: 4px;">#${p.id}</code>
                        </div>
...
async function createProject() {
    ...
    // === 主动学习新增：获取新建项目时的文本提示 ===
    const promptElement = document.getElementById('newProjectPrompt');
    const textPrompt = promptElement ? promptElement.value.trim() : ''; 
    // ===========================================
    ...
    try {
        const response = await fetch('/api/project/create', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            // === 主动学习修改：在 body 中追加 text_prompt 字段 ===
            body: JSON.stringify({ 
                name, 
                image_dir: imageDir, 
                output_dir: outputDir, 
                classes,
                text_prompt: textPrompt // <--- 新增这行
            })
            // =================================================
        });
...
async function selectProject(projectId) {
    try {
        ...
        if (data.success) {
            ...
            // 记录初始归属，防止在操作时图片在列表间跳来跳去
            // state.images = (data.project.images || []).map(img => {
            //     img._displayTab = data.project.image_states?.[img.filename] || img.status || 'unannotated';
            //     return img;
            // });
            // 引入“本地强力锚点”，抵御 F5 刷新乱跳
            const savedTabsStr = localStorage.getItem(`sam3_tabs_${state.projectId}`);
            const savedTabs = savedTabsStr ? JSON.parse(savedTabsStr) : null;

            state.images = (data.project.images || []).map(img => {
                // 1. 优先读取浏览器缓存的“本轮锁”
                if (savedTabs && savedTabs[img.filename]) {
                    img._displayTab = savedTabs[img.filename];
                } else {
                    // 2. 【核心修复】没锁时，绝对优先读取后端的动态状态 (image_states)！
                    img._displayTab = data.project.image_states?.[img.filename] || img.status || 'unannotated';
                }
                return img;
            });
            ...
            document.getElementById('projectName').innerHTML = `${data.project.name} <small class="text-muted" style="font-size: 0.8rem; font-weight: normal;">(ID: ${projectId})</small>`;
...
function updateImageList() {
    const lists = ['listManual', 'listReview', 'listPool', 'listDone'];
    lists.forEach(id => {
        const el = document.getElementById(id);
        if (el) el.innerHTML = '';
    });

    const imageStates = state.projectData?.image_states || {};
    const annotatedCount = state.images.filter(img => imageStates[img.filename] === 'completed').length;
    const total = state.images.length;
    const percent = total > 0 ? Math.round(annotatedCount / total * 100) : 0;

    // 更新统计和进度条
    const statsEl = document.getElementById('imageStats');
    if (statsEl) statsEl.textContent = `${annotatedCount}/${total}`;
    const barEl = document.getElementById('progressBar');
    const textEl = document.getElementById('progressText');
    if (barEl) barEl.style.width = `${percent}%`;
    if (textEl) textEl.textContent = `已标注: ${annotatedCount}/${total} (${percent}%)`;

    if (total === 0) {
        const poolEl = document.getElementById('listPool');
        if (poolEl) poolEl.innerHTML = '<div class="empty-state"><i class="bi bi-images"></i><p>暂无图片</p></div>';
        return;
    }

    state.images.forEach((img, idx) => {
        // 1. 当前真实状态（决定它的字是什么颜色）
        const currentState = imageStates[img.filename] || 'unannotated';
        // 2. 本轮所在的池（决定它呆在哪个 UI 标签页里不走）
        const tabTarget = img._displayTab || 'unannotated';

        // 3. 决定文字颜色
        let colorClass = '';
        if (currentState === 'completed') {
            colorClass = 'text-success fw-bold'; // 绿色：已确认/已保存
        } else if (currentState === 'unannotated' || currentState === 'rejected') {
            if (tabTarget === 'review' || tabTarget === 'pending_hard' || tabTarget === 'manual_batch') {
                colorClass = 'text-danger fw-bold'; // 红色：被打回/被清空
            }
        }

        const item = document.createElement('div');
        // item.className = `image-item ${idx === state.currentIndex ? 'active' : ''} ${img.annotated ? 'annotated' : ''}`;
        item.className = `image-item ${idx === state.currentIndex ? 'active' : ''}`;
        item.onclick = () => loadImage(idx);

        let badgeHtml = '';
        if (tabTarget === 'review' || tabTarget === 'pending_hard') {
            badgeHtml = '<span class="badge bg-warning text-dark ms-2" style="font-size:0.7rem;">待检</span>';
        }

        item.innerHTML = `
            <span class="index">${idx + 1}</span>
            <span class="filename ${colorClass}">${img.filename}</span>
            ${badgeHtml}
        `;

        // 4. 根据固定的 tabTarget 分发DOM，确保点完不会跳走
        if (tabTarget === 'manual_batch') {
            document.getElementById('listManual')?.appendChild(item);
        } else if (tabTarget === 'review' || tabTarget === 'pending_hard') {
            document.getElementById('listReview')?.appendChild(item);
        } else if (tabTarget === 'completed') {
            document.getElementById('listDone')?.appendChild(item);
        } else {
            document.getElementById('listPool')?.appendChild(item);
        }
    });
}
...
async function loadImage(index) {
    ...
    currentImage.onload = () => {
        ...
        const currentState = state.projectData?.image_states?.[img.filename] || img.status || 'unannotated';
        const displayTab = img._displayTab || currentState;
        
        // 判断当前图片是否已经被人工处理过（变绿或变红了）
        const isFinished = (currentState === 'completed' || currentState === 'rejected');
        
        updateActionButtons(displayTab, isFinished);
        // =======================
    };
...
async function saveAnnotations(showMessage = true, skipStateChange = false, forceCompleted = false) {
    if (!state.projectId || state.currentIndex < 0) return;
    ...
    try {
        ...
        if (data.success) {
            ...
            img.annotations = [...state.annotations];
            img.annotated = state.annotations.length > 0;
            const imgName = img.filename;

            if (!state.projectData.image_states) state.projectData.image_states = {};

            // 核心修复：绝对不越权改状态！
            if (!skipStateChange) {
                if (img._displayTab === 'manual_batch') {
                    // 待标池：如果有框，或者点击了“无目标”强制完成，都设为 completed。如果是单纯被清空了且没点无目标，就是 rejected(变红)
                    const newState = (img.annotated || forceCompleted) ? 'completed' : 'rejected';
                    state.projectData.image_states[imgName] = newState;
                    fetch(`/api/projects/${state.projectId}/images/${imgName}/state`, {
                        method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ state: newState })
                    });
                    updateActionButtons('manual_batch', true);
                    
                    if (showMessage) {
                        if (forceCompleted) {
                            showToast('已保存', '已作为【无目标负样本】保存，参与训练可有效降低误检率！', 'info');
                        } else {
                            showToast('已保存', '记录已更新，点击下一张继续。');
                        }
                    }
                } else if (img._displayTab !== 'review' && img._displayTab !== 'pending_hard') {
                    // 普通未标注池：同理，支持无目标图
                    const newState = (img.annotated || forceCompleted) ? 'completed' : 'unannotated';
                    state.projectData.image_states[imgName] = newState;
                    fetch(`/api/projects/${state.projectId}/images/${imgName}/state`, {
                        method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ state: newState })
                    });
                    ...
                    if (showMessage) {
                        if (forceCompleted) {
                            showToast('成功', '已作为【无目标负样本】保存入库！', 'info');
                        } else {
                            showToast('成功', '保存成功！');
                        }
                    }
                } else {
                    // 待检池：调整了框并保存，只存坐标，必须保持待检状态
                    if (showMessage) showToast('成功', '坐标已更新 (状态保留，请点击确认或打回)');
                }
            }
...
// ==========================================================
// 主动学习 (Active Learning & DEAC) 专属控制逻辑
// ==========================================================
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (threshSlider) {
        threshSlider.addEventListener('input', e => {
            document.getElementById('threshLabel').innerText = e.target.value;
        });
    }
    ...
    if (epochSlider) {
        epochSlider.addEventListener('input', e => {
            document.getElementById('epochLabel').innerText = e.target.value;
        });
    }
});
...
async function fetchNextBatch() {
    if (!state.projectId) return showToast('提示', '请先选择或创建项目', 'danger');
    try {
        const res = await fetch(`/api/projects/${state.projectId}/next-batch`, { method: 'POST' });
        const data = await res.json();
        if (data.success) {
            showToast('成功', `成功抽出 ${data.batch.length} 张图片到本轮待标池！`, 'success');

            localStorage.removeItem(`sam3_tabs_${state.projectId}`);
        ...
        showToast('错误', '抽取难例失败', 'danger');
    }
}

// 2. 一键触发：提示优化(Train) + 机器自动检验(Infer)
async function runTrainAndInfer() {
    if (!state.projectId) return showToast('提示', '请先选择项目', 'danger');
    
    const thresh = document.getElementById('threshSlider') ? document.getElementById('threshSlider').value : 0.6;
    const epochs = document.getElementById('epochSlider') ? parseInt(document.getElementById('epochSlider').value) : 50;
    const lr = document.getElementById('lrInput') ? parseFloat(document.getElementById('lrInput').value) : 0.0005;

    if (!confirm(`即将使用您刚刚标注的图作为先验知识优化模型。\n[配置] Epochs: ${epochs}, LR: ${lr}\n随后模型将自动扫描未标图片(阈值:${thresh})。\n该过程可能需要几分钟，是否继续？`)) return;
    try {
        // 第一步：先显示转圈圈动画 (传入 progress = -1 会触发你的 loading-pulse CSS 动画)
        showLoading('正在进行视觉概念搜索，请耐心等待...', -1);
        
        // 触发后台任务
        await fetch(`/api/projects/${state.projectId}/train_and_infer`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ 
                solved_thresh: thresh,
                epochs: epochs,
                lr: lr
            })
        });
        
        // 开启轮询：每隔 1 秒向后端查询一次进度
        const timer = setInterval(async () => {
            try {
                const res = await fetch(`/api/projects/${state.projectId}/al_progress`);
                ...
                if (json.success && json.data) {
                    ...
                    if (task.status === 'training') {
                        // 正在训练，继续转圈圈
                        showLoading('视觉概念搜索中... ', -1);
                    } 
                    else if (task.status === 'inferring') {
                        // 进入推理阶段，切换成真实的进度条！
                        const percent = task.total > 0 ? (task.current / task.total) * 100 : 0;
                        showLoading(`正在进行自动标注... ${task.current} / ${task.total}`, percent);
                    } 
                    else if (task.status === 'completed') {
                        // 任务彻底完成
                        clearInterval(timer);
                        hideLoading();
                        showToast('完成', '自动标注完成！', 'success');

                        localStorage.removeItem(`sam3_tabs_${state.projectId}`);

                        refreshProjectState(); // 无感刷新列表
                    } 
                    else if (task.status === 'error') {
                        // 发生报错
                        clearInterval(timer);
                        hideLoading();
                        showToast('错误', '后台处理发生异常: ' + task.message, 'danger');
                    }
                }
            } catch (err) {
                console.error("查询进度失败:", err);
                // 这里不 clear timer，允许网络抖动
            }
        }, 1000); // 1000 毫秒 = 1 秒查一次

    } catch (error) {
        hideLoading();
        showToast('错误', '启动主动学习引擎失败', 'danger');
    }
}

// 3. 通用状态修改器
async function changeImgState(newState) {
    if (!state.images.length || state.currentIndex < 0) return;
    const imgName = state.images[state.currentIndex].filename;
    try {
        await fetch(`/api/projects/${state.projectId}/images/${imgName}/state`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
        ...
        // 注意：这里不再处理按钮显示逻辑，交给了 rejectImage / acceptImage
    } catch (error) {
        showToast('错误', '更新图片状态失败', 'danger');
    }
}

async function acceptImage() {
    await changeImgState('completed');
    await saveAnnotations(false, true); // 纯保存坐标，跳过状态干涉
    const img = state.images[state.currentIndex];
    updateActionButtons(img._displayTab, true);
    showToast('完成', '已确认无误，移入已完成池！', 'success');
}

async function rejectImage() {
    await changeImgState('rejected');
    const img = state.images[state.currentIndex];
    updateActionButtons(img._displayTab, true);
    showToast('已打回', '该图片已被打回重标！', 'info');
}
// async function acceptImage() {
//     // 1. 先调用保存（此时因为模拟阶段画板是空的，saveAnnotations 会暂时把它当成 unannotated）
//     await saveAnnotations(false);  
    
//     // 2. 然后我们强制把状态改回 completed！(这步在后，会完美覆盖掉上面因为空画布导致的误判)
//     await changeImgState('completed'); 
    
//     showToast('完成', '已确认无误，数据已保存到已完成池！', 'success');
// }

// 6. 无感刷新项目状态 (不刷新整个网页，只更新列表和按钮)
async function refreshProjectState() {
    ...
    try {
        ...
        if (data.success) {
            ...
            // 重新校验当前展示图片的下方按钮状态 (打回/确认)
            if (state.images.length > 0 && state.currentIndex >= 0) {
                const img = state.images[state.currentIndex];
                const imgName = img.filename;
                const imgState = state.projectData.image_states?.[imgName] || 'unannotated';
                
                updateActionButtons(img._displayTab || imgState, false);
            }
        }
    } catch (error) {
...
// 7. 监听标签页(Tab)切换事件，实现真正的“自动化”
document.addEventListener('DOMContentLoaded', () => {
    // 找到我们加的 4 个 Tab 按钮
    const tabEls = document.querySelectorAll('#alTabs a[data-bs-toggle="tab"]');
    tabEls.forEach(tabEl => {
        // Bootstrap 5 的标签页切换事件
        tabEl.addEventListener('shown.bs.tab', function (event) {
            // 用户只要点击了任意一个 Tab (比如点击"待检验")，立刻无感拉取后台最新数据！
            refreshProjectState();
        });
    });
});
...
// ==========================================================
// 按钮显示统一管理
// finished: 如果是刚点完打回/确认/保存，则为 true，用来显示“下一张”并隐藏“保存”
// ==========================================================
function updateActionButtons(displayTab, finished = false) {
    const btns = {
        reject: document.getElementById('btnReject'),
        accept: document.getElementById('btnAccept'),
        nextReview: document.getElementById('btnNextReview'),
        nextManual: document.getElementById('btnNextManual'),
        save: document.getElementById('btnSaveNormal'),
        noTarget: document.getElementById('btnNoTarget')
    };

    // 先全部隐藏
    Object.values(btns).forEach(b => { if(b) b.style.display = 'none'; });

    if (displayTab === 'review' || displayTab === 'pending_hard') {
        if (finished) {
            if(btns.nextReview) btns.nextReview.style.display = 'inline-block';
        } else {
            if(btns.reject) btns.reject.style.display = 'inline-block';
            if(btns.accept) btns.accept.style.display = 'inline-block';
        }
    } else if (displayTab === 'manual_batch') {
        if (finished) {
            if(btns.nextManual) btns.nextManual.style.display = 'inline-block';
        } else {
            if(btns.save) btns.save.style.display = 'inline-block';
            if(btns.noTarget) btns.noTarget.style.display = 'inline-block'; // <--- 在未完成时显示
        }
    } else {
        if(btns.save) btns.save.style.display = 'inline-block';
    }
...
// 自动寻找下一张图片的逻辑
// ==========================================================
function autoNextReview() {
    const found = findNextInPool('review') || findNextInPool('pending_hard');
    if (!found) {
        showToast('太棒了', '当前所有待检验图片已处理完毕！', 'success');
        updateActionButtons('completed', false);
    }
}

function autoNextManual() {
    const found = findNextInPool('manual_batch');
    if (!found) {
        showToast('太棒了', '本轮待标图片已全部处理完毕！', 'success');
        updateActionButtons('completed', false);
    }
}

// 通用的池搜索算法
function findNextInPool(targetPool) {
    const imageStates = state.projectData?.image_states || {};
    // 1. 从当前往后找
    for (let i = state.currentIndex + 1; i < state.images.length; i++) {
        const img = state.images[i];
        if (img._displayTab === targetPool) {
            const currStatus = imageStates[img.filename] || 'unannotated';
            // 只要还没确认(completed)且没被打回/清空(rejected)，就可以看
            if (currStatus !== 'completed' && currStatus !== 'rejected') {
                loadImage(i);
                return true;
            }
        }
...
// ==========================================================
// 缓存当前图片的 Tab 归属，防止 F5 刷新导致状态被后端的 completed 覆盖而乱跳
// ==========================================================
function saveTabState() {
    ...
    state.images.forEach(img => {
        if (img._displayTab) {
            tabMap[img.filename] = img._displayTab;
        }
    });
    localStorage.setItem(`sam3_tabs_${state.projectId}`, JSON.stringify(tabMap));
}

// ==================== 隐藏/显示标注 (小眼睛) ====================
function toggleVisibility() {
    state.showAnnotations = !state.showAnnotations;
    const icon = document.querySelector('#btnToggleVis i');
    const btn = document.getElementById('btnToggleVis');
    
    if (icon && btn) {
        // 切换图标
        icon.className = state.showAnnotations ? 'bi bi-eye' : 'bi bi-eye-slash';
    ...
    invalidateStaticCache(); // 强制更新静态缓存
    redraw(); // 重绘
}

// 将当前图片标记为无目标的负样本背景图
async function markAsEmptyBackground() {
    state.annotations = []; // 清空可能残留的点或框
    invalidateStaticCache();
    redraw();
    // 强制按 completed 状态保存 (第3个参数 forceCompleted = true)
    await saveAnnotations(true, false, true); 
}

/**
 * 💡 新增功能：点击箭头微调滑动条的值
 * @param {string} sliderId - 滑动条的 ID
 * @param {number} direction - 1 为增大，-1 为减小
 */
function stepSlider(sliderId, direction) {
    ...
    // 更新滑动条的值 (处理浮点数精度问题)
    slider.value = val.toFixed(slider.step.includes('.') ? slider.step.split('.')[1].length : 0);
    
    // 核心：强制触发 'input' 事件！
    // 这样你原本写在 JS 里的 "滑动条变动 -> 改变 b 标签数字" 的逻辑就会被自动激活
